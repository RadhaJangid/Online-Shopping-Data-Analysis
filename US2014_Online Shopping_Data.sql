...
# (1). Customer Segmentation Based on Spend in Dollars, based on Swipes, segmentation??
...
# (2). Calculate the spend in terms of Product, State and Payment method?
		SELECT P_Category AS Product_Category, ROUND(SUM(Selling_price),3) AS Total_Spend FROM Customer_Purchase  GROUP BY P_Category ORDER BY Total_Spend DESC;
		SELECT State, ROUND(SUM(Selling_price),3) AS Total_Spend FROM Customer_Purchase  GROUP BY State ORDER BY Total_Spend DESC;
		SELECT Payment_Method, ROUND(SUM(Selling_price),3) AS Total_Spend FROM Customer_Purchase  GROUP BY Payment_Method ORDER BY Total_Spend DESC;    
        
# (3). Calculate the highest 5 spending in all above categories.   
		SELECT Name, ROUND(SUM(Selling_price),2) Spending FROM Customer_Purchase  GROUP BY Name ORDER BY Spending DESC LIMIT 5;
    
# (4). Which payment method is providing more discount for customers?
		ALTER TABLE Customer_Purchase ADD Discount double;
			UPDATE Customer_Purchase SET Discount = Price - Selling_Price
								WHERE Discount IS NULL;  
		SELECT Payment_Method , SUM(Discount) Total_Discount FROM Customer_Purchase GROUP BY Payment_Method ORDER BY  Total_Discount DESC LIMIT 1;

# (5). Total number of orders
		SELECT Credit_Card AS Order_ID, COUNT(Credit_Card) Total_Orders FROM Customer_Purchase GROUP BY Credit_Card;
    
# (6). Create a profile for high value items vs low value items and relate that wrt to their number of orders.
		# 1 - High_value_items
		# 0 - Low_value_items
        SELECT Value_Items , COUNT(Credit_Card) "number of orders" FROM Customer_Purchase GROUP BY Value_Items;
